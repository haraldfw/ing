...
class GodkjenningBGS {
	...
	private AssignmentOverview oversikt;
	public GodkjenningBGS(AssignmentOverview oversikt) {
	...
	public String lesValg() {
		int antStud = oversikt.getStudentAmount();
		String valg = (String) showInputDialog(null, "Velg fra listen, " + antStud + " studenter:",  "ArrayList",
	...
	private void registrerNyStudent() {
		...
		if (oversikt.addStudent(navnNyStud)) {
			showMessageDialog(null, navnNyStud + " er registrert.");
			String[] alleNavn = oversikt.allNames();
	...
	private void registrerOppgaver(String studNavn) {
		...
		do { // gjentar inntil registrering aksepteres av objektet oversikt
			try {
				...
				oversikt.incrementAssign(studNavn, antOppgØkning);  // kan ikke returnere false, pga navn alltid gyldig
		...
		melding = "Totalt antall oppgaver registrert på " + studNavn + " er " + oversikt.assignmentsCompleted(studNavn) + ".";
...
class Oppgave1 {
	public static void main(String[] args) {
		...
		AssignmentOverview oversikt = new AssignmentOverview();
